...
async function getData() {
  ...
  const top5Teams = [...allTeamMap.entries()]
    .map(([name, scores]) => ({ name, avg: scores.length ? scores.reduce((a, b) => a + b, 0) / scores.length : 0 }))
  ...
  const allPastorTeams = [...new Set(
    pastors.flatMap(p => p.serviceTeams.map(t => t.name))
  )]
  const pastorGroupData: PastorBarEntry[] = pastors
    .filter(p => p.serviceTeams.length > 0)
    .map(p => {
      ...
      for (const team of p.serviceTeams) {
        ...
        entry[team.name] = scores.length ? scores.reduce((a, b) => a + b, 0) / scores.length : null
      }
