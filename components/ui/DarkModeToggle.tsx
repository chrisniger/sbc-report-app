...
export default function DarkModeToggle() {
  ...
  return (
    ...
    >
      ...
      <div className="relative h-7 w-14 rounded-full bg-sbc-red shadow-inner shadow-sbc-red/20 transition-colors dark:bg-white/10">
        <div
          className={`absolute left-1 top-1 h-5 w-5 rounded-full bg-white shadow-[0_2px_10px_rgba(0,0,0,0.25)] transition-transform duration-200 ${isDark ? 'translate-x-7' : 'translate-x-0'}`}
        />
      </div>
    </button>
  )
